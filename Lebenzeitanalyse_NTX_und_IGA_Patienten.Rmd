...
daten_ntx %>% 
  mutate(transplantatverlsut = as.factor(case_when(
    !is.na(daten_ntx$T_Ende) ~ 0,
     is.na(daten_ntx$T_Ende) ~ 1
  ))) %>% 
...
daten_ntx %>% 
  mutate(
    T_Ende = case_when(
      !is.na(T_Ende) & T_Ende > follow_up ~ follow_up,
      !is.na(T_Ende) & T_Ende <= follow_up ~ T_Ende,
      is.na(T_Ende) ~ follow_up
    )) %>% 
  ggplot() +
...
daten_ntx %>% 
  filter(!is.na(T_Ende) & (T_Ende < follow_up) ) 
  ggplot() +
    geom_boxplot(aes(y = as.numeric(difftime(daten_ntx$T_Ende,daten_ntx$Datum), units = "days") / 365.25,
                     x = daten_ntx$Geschlecht)) +
    ylab("Zeit bis Transplantatversagen") +
    xlab("Geschlecht")+ 
    ggtitle("Zeitverlauf Transplantatversagen NTX Patienten",
